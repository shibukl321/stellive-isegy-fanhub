...
function initGoogle(){
  const btn = q('#btnLogin'); if(!btn) return;
  if(!window.google || !window.google.accounts || !window.GOOGLE_AUDIENCE) return;
  window.google.accounts.id.initialize({
    client_id: window.GOOGLE_AUDIENCE,
    callback: async (res)=>{
      try{
...
async function loadAdmin(){
  ...
  q('#btnLiveSave').addEventListener('click', async ()=>{
    ...
    const r=await fetch(window.BACKEND_BASE + '/live', {method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({key,on})});
    const j=await r.json(); alert(j.ok?'저장됨':'실패');
  });
  q('#btnSeedAdd').addEventListener('click', async ()=>{
    const r=await fetch(window.BACKEND_BASE + '/gallery/seed', {method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:sel2.value,url:q('#seedUrl').value})});
    const j=await r.json(); alert(j.ok?'추가됨':'실패');
  });
  q('#btnPinSet').addEventListener('click', async ()=>{
    const r=await fetch(window.BACKEND_BASE + '/gallery/pin', {method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:sel3.value,url:q('#pinUrl').value})});
...
(async function(){
  ...
  qa('.tabs .tab').forEach(a=>{ if(a.getAttribute('href')===path) a.classList.add('active'); });
  ...
  const btnLogout = q('#btnLogout'); if(btnLogout) btnLogout.addEventListener('click', logout);
})();
